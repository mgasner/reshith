...
import {
  GET_SANSKRIT_DECLENSION_EXERCISES,
  GRADE_SANSKRIT_EXERCISE,
...
export function SanskritDeclensionExercisePage() {
  ...
  const { data, loading, refetch } = useQuery(GET_SANSKRIT_DECLENSION_EXERCISES, {
    variables: { count: 10, maxLesson },
    fetchPolicy: 'network-only',
  })

  const [gradeExercise] = useMutation(GRADE_SANSKRIT_EXERCISE)

  const exercises: Exercise[] = data?.sanskritDeclensionExercises || []
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const gradeResult = result.data?.gradeSanskritExercise
      if (gradeResult) {
  ...
  return (
    <div className="px-4 max-w-2xl mx-auto">
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-gray-900">Sanskrit Noun Declension</h1>
      ...
      <div className="mb-6 flex flex-wrap gap-4">
        <div>
          ...
          >
            <option value={1}>Lesson 1 (a-stem nouns)</option>
            <option value={2}>Lessons 1–2 (i-stem &amp; u-stem)</option>
      ...
      <div className="bg-white rounded-xl shadow-lg p-6 mb-6">
        <div className="text-center mb-6">
          <p className="text-sm text-gray-500 mb-1">Dictionary form (IAST)</p>
          <p className="text-2xl font-serif font-semibold">{currentExercise.dictForm}</p>
          {currentExercise.devanagari && (
            <p className="text-2xl mt-1" style={{ fontFamily: 'serif' }}>
              {currentExercise.devanagari}
            </p>
          )}
          <p className="text-sm text-gray-500 italic mt-1">{currentExercise.definition}</p>
          <p className="text-lg font-medium text-blue-700 mt-4">{currentExercise.prompt}</p>
        </div>
        ...
        <div className="mb-4">
          <input
            ...
            placeholder="Type the declined form in IAST (e.g. nara, narāya)..."
          ...
          <p className="text-xs text-gray-400 mt-1">
            Enter IAST transliteration (e.g. ā, ī, ū for long vowels; ṭ, ḍ, ṇ, ś, ṣ for other characters)
          </p>
        </div>
