...
const LXX_CHAPTER = gql`
  query LXXChapter($book: String!, $chapter: Int!) {
    lxxChapter(book: $book, chapter: $chapter) {
      ref
      book
      chapter
      verse
...
const LXX_CHAPTER_TRANSLATIONS = gql`
  query LXXChapterTranslations($book: String!, $chapter: Int!) {
    lxxChapterTranslations(book: $book, chapter: $chapter) {
      verse
      text
    }
  }
...
const LXX_SEARCH = gql`
  query LXXSearch($query: String!, $limit: Int) {
    lxxSearch(query: $query, limit: $limit) {
      ref
      book
      chapter
      verse
...
export function LXXViewerPage() {
  const [searchParams, setSearchParams] = useSearchParams()
  const [mode, setMode] = useState<'browse' | 'search'>('browse')
  const [selectedBook, setSelectedBook] = useState(() => searchParams.get('book') ?? 'Gen')
  ...
  const { data: booksData, loading: booksLoading } = useQuery(LXX_BOOKS)
  const books: BookInfo[] = booksData?.lxxBooks ?? []
  const currentBook = books.find((b) => b.abbrev === selectedBook)

  const [loadChapter, { data: chapterData, loading: chapterLoading }] =
    useLazyQuery(LXX_CHAPTER)
  const [loadTranslations, { data: translationsData }] =
    useLazyQuery(LXX_CHAPTER_TRANSLATIONS)
  const [runSearch, { data: searchData, loading: searchLoading }] =
    useLazyQuery(LXX_SEARCH)

  const translationMap = new Map<number, string>()
  for (const t of (translationsData?.lxxChapterTranslations ?? []) as VerseTranslation[]) {
  ...
  const tokens: GreekToken[] = chapterData?.lxxChapter ?? []
  ...
  const searchResults: GreekToken[] = searchData?.lxxSearch ?? []
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8">
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-stone-800">Septuagint (LXX)</h1>
        <p className="text-sm text-stone-500 mt-1">
          Rahlfs LXX · STEPBible TALXX morphology — interlinear word analysis
        </p>
      </div>
        ...
        <>
          <div className="flex flex-wrap items-center gap-3 mb-6">
            <select
              ...
              className="border border-stone-300 rounded-md px-3 py-1.5 text-sm bg-white text-stone-800 focus:outline-none focus:ring-2 focus:ring-emerald-400"
            ...
            <div className="flex items-center gap-1">
              ...
              <select
                ...
                className="border border-stone-300 rounded-md px-2 py-1.5 text-sm bg-white text-stone-800 focus:outline-none focus:ring-2 focus:ring-emerald-400"
              >
                {currentBook &&
            ...
            <button
              ...
              title="Toggle Brenton translation"
            >
        ...
        <>
          ...
          >
            <input
              ...
              className="flex-1 border border-stone-300 rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-400"
            />
            <button
              type="submit"
                    ...
                    {searchResults.map((tok) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <td className="py-1.5 pr-4 text-emerald-600 font-mono text-xs">{tok.dstrongs}</td>
      ...
      <div className="mt-12 pt-4 border-t border-stone-100 text-xs text-stone-400">
        Rahlfs LXX text · STEPBible TALXX morphology (CC BY 4.0) · Translation: Brenton 1851 (public domain) · Click any word to see full analysis
      </div>
    </div>
  )
}
