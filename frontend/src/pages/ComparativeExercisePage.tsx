...
import { GET_COMPARATIVE_EXERCISES, GRADE_COMPARATIVE_EXERCISE } from '@/graphql/operations'

interface ComparativeExercise {
  id: string
  pattern: string
  hebrew: string
...
export function ComparativeExercisePage() {
  const [maxLesson, setMaxLesson] = useState(5)

  const { data, loading, refetch } = useQuery(GET_COMPARATIVE_EXERCISES, {
    variables: { count: 10, maxLesson },
    fetchPolicy: 'network-only',
  })

  const [gradeExercise] = useMutation(GRADE_COMPARATIVE_EXERCISE)

  const exercises: ComparativeExercise[] = data?.comparativeExercises ?? []

  return (
    <ExerciseRunner
      title="Comparative Exercises"
      description="Translate Hebrew comparative constructions using מִן to English."
      exercises={exercises}
      loading={loading}
      onRefetch={() => refetch()}
      inputMode="english"
      ...
      onGrade={async (ex, answer) => {
        ...
        const r = result.data?.gradeComparativeExercise
        return {
          correct: r.correct,
          feedback: r.feedback,
          expected: r.expected,
