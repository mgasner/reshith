...
import { GET_RELATIVE_CLAUSE_EXERCISES, GRADE_RELATIVE_CLAUSE_EXERCISE } from '@/graphql/operations'

interface RelativeClauseExercise {
  id: string
  pattern: string
  hebrew: string
...
export function RelativeClauseExercisePage() {
  const [maxLesson, setMaxLesson] = useState(5)

  const { data, loading, refetch } = useQuery(GET_RELATIVE_CLAUSE_EXERCISES, {
    variables: { count: 10, maxLesson },
    fetchPolicy: 'network-only',
  })

  const [gradeExercise] = useMutation(GRADE_RELATIVE_CLAUSE_EXERCISE)

  const exercises: RelativeClauseExercise[] = data?.relativeClauseExercises ?? []

  return (
    <ExerciseRunner
      title="Relative Clause Exercises"
      description="Translate Hebrew relative clauses using אֲשֶׁר to English."
      exercises={exercises}
      loading={loading}
      onRefetch={() => refetch()}
      inputMode="english"
      ...
      onGrade={async (ex, answer) => {
        ...
        const r = result.data?.gradeRelativeClauseExercise
        return {
          correct: r.correct,
          feedback: r.feedback,
          expected: r.expected,
