...
import {
  GET_GREEK_CONJUGATION_EXERCISES,
  GRADE_GREEK_EXERCISE,
...
export function NTGreekConjugationExercisePage() {
  ...
  const { data, loading, refetch } = useQuery(GET_GREEK_CONJUGATION_EXERCISES, {
    variables: { count: 10, maxLesson, variant: 'KOINE' },
    fetchPolicy: 'network-only',
  })

  const [gradeExercise] = useMutation(GRADE_GREEK_EXERCISE)

  const exercises: Exercise[] = data?.greekConjugationExercises || []
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const gradeResult = result.data?.gradeGreekExercise
      if (gradeResult) {
  ...
  return (
    <div className="px-4 max-w-2xl mx-auto">
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-gray-900">NT Greek Verb Conjugation</h1>
      ...
      <div className="mb-6 flex flex-wrap gap-4">
        <div>
          ...
          >
            <option value={1}>Lesson 1 (thematic verbs)</option>
      ...
      <div className="bg-white rounded-xl shadow-lg p-6 mb-6">
        ...
        <div className="mb-4">
          ...
          <p className="text-xs text-gray-400 mt-1">
            Use Greek characters or beta-code transliteration
          </p>
        </div>
