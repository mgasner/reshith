...
import {
  GET_LATIN_CONJUGATION_EXERCISES,
  GRADE_LATIN_CONJUGATION_EXERCISE,
...
export function EcclesiasticalLatinConjugationExercisePage() {
  ...
  const { data, loading, refetch } = useQuery(GET_LATIN_CONJUGATION_EXERCISES, {
    variables: { count: 10, maxLesson, variant: 'ECCLESIASTICAL' },
    fetchPolicy: 'network-only',
  })

  const [gradeExercise] = useMutation(GRADE_LATIN_CONJUGATION_EXERCISE)

  const exercises: Exercise[] = data?.latinConjugationExercises || []
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const gradeResult = result.data?.gradeLatinConjugationExercise
      if (gradeResult) {
  ...
  return (
    <div className="px-4 max-w-2xl mx-auto">
      <div className="mb-6">
        ...
        <h1 className="text-2xl font-bold text-gray-900">Ecclesiastical Latin — Verb Conjugation</h1>
        <p className="text-gray-600">
          Produce the correct conjugated form for the given person and number.
        </p>
      </div>
      ...
      <div className="mb-6 flex flex-wrap gap-4">
        <div>
          ...
          >
            <option value={1}>Lesson 1 (1st &amp; 2nd conjugation)</option>
            <option value={2}>Lessons 1–2 (more verbs)</option>
      ...
      <div className="bg-white rounded-xl shadow-lg p-6 mb-6">
        <div className="text-center mb-6">
          <p className="text-sm text-gray-500 mb-1">Verb</p>
          <p className="text-2xl font-serif font-semibold">{currentExercise.dictForm}</p>
          <p className="text-sm text-gray-500 italic mt-1">{currentExercise.definition.split(';')[0]}</p>
        ...
        <div className="mb-4">
          ...
          <p className="text-xs text-gray-400 mt-1">
            Macrons optional (e.g. "amas" accepted for "amās")
          </p>
        </div>
