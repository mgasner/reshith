...
export function VowelsPage() {
  ...
  useEffect(() => {
    fetch('/data/hebrew/vowels.json')
      ...
      .catch((err) => {
        console.error('Failed to load vowels data:', err)
        setLoading(false)
      })
  }, [])
  ...
  if (loading) {
    return (
      <div className="px-4 text-center py-12">
        <p className="text-gray-500">Loading vowels...</p>
      </div>
    )
  }
  ...
  if (!deck) {
    return (
      <div className="px-4 text-center py-12">
        <p className="text-red-500">Failed to load vowels data</p>
      </div>
    )
  }
