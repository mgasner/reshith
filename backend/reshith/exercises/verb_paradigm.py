...
# Reference Qal paradigm (שָׁמַר) in template order. This is the vocalisation
# pattern the strong-verb generator (strong_verb.py) projects onto other
# regular triliteral roots. Keep it byte-for-byte identical to the שׁמר entry
# in _VERB_DATA below (test_strong_verb.py enforces this).
_QAL_REFERENCE: list[tuple[str, str]] = [
    # Perfect
...
# ── Generated strong-verb Qal paradigms ──────────────────────────────────────
# Regular triliteral roots share the שָׁמַר vocalisation pattern, so their Qal
# paradigms are generated by strong_verb.generate_qal_forms rather than typed by
# hand. Only genuinely irregular cells need per-verb overrides. Everything here
# is verified against the hand-entered reference verbs in test_strong_verb.py.

# Template indices whose C2 shewa is vocal (so C3 takes no dagesh lene): the Qal
# fs/mp imperatives (e.g. כִּתְבִי, כִּתְבוּ). Derived from the Qal template.
_QAL_VOCAL_SHEWA_C3 = frozenset(
    i
    for i, (conj, _p, number, gender, _label) in enumerate(BINYANIM["qal"]["template"])
    if conj == "imperative"
    and ((number == "sg" and gender == "f") or (number == "pl" and gender == "m"))
)
...
def _strong_qal(
    ...
    """Register a generated Qal paradigm for a regular (strong) triliteral root."""
    from reshith.exercises.strong_verb import generate_qal_forms

    forms = generate_qal_forms(
        radicals, vocal_shewa_c3_indices=_QAL_VOCAL_SHEWA_C3
    )
    citation, citation_translit = forms[0]
    _VERB_DATA[("qal", root)] = {
        "root_transliteration": root_transliteration,
        "citation": citation,
        "citation_transliteration": citation_translit,
        "definition": definition,
        "forms": forms,
    }


# Additional high-frequency strong verbs. Radicals are given explicitly so the
# generator never has to guess where a root string splits.
_STRONG_QAL_VERBS: list[tuple[str, tuple[str, str, str], str, str]] = [
    ("זכר", ("ז", "כ", "ר"), "z-k-r", "to remember, mention, be mindful of"),
    ("מכר", ("מ", "כ", "ר"), "m-k-r", "to sell, hand over"),
    ("קבר", ("ק", "ב", "ר"), "q-b-r", "to bury"),
    ("רדף", ("ר", "ד", "פ"), "r-d-p", "to pursue, chase, persecute"),
    ("סגר", ("ס", "ג", "ר"), "s-g-r", "to shut, close"),
    ("דרשׁ", ("ד", "ר", "\u05e9"), "d-r-š", "to seek, inquire, require"),
    ("קשׁר", ("ק", "\u05e9", "ר"), "q-š-r", "to bind, tie, conspire"),
    ("משׁל", ("מ", "\u05e9", "ל"), "m-š-l", "to rule, have dominion"),
]

for _root, _radicals, _rt, _defn in _STRONG_QAL_VERBS:
    _strong_qal(_root, _radicals, _rt, _defn)
